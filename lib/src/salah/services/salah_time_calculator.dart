...
import 'package:myapp/src/models/config/models.dart';
import 'package:myapp/src/models/salah_times.dart';
import 'package:myapp/src/services/sun_timetable_calculator.dart';

class SalahTimeCalculator {
  static Future<SalahTimes> calculate(DateTime date, double latitude, double longitude, Method method, {Madhab? madhab}) async {
    final madhabToUse = madhab ?? method.madhab;
    final madhabValue = madhabToUse == Madhab.hanafi ? 2 : 1;

    final sunTimetable = SunTimetableCalculator.calculate(date, latitude, longitude);

    final fajr = _calculateTimeForAngle(date, latitude, longitude, method.fajrAngle);

    DateTime isha;
    if (method.ishaAngle != null) {
      isha = _calculateTimeForAngle(date, latitude, longitude, method.ishaAngle!, ishan: true);
    } else {
      isha = sunTimetable.sunsetEnd.add(Duration(minutes: method.ishaInterval ?? 90));
    }

    DateTime maghrib;
    if(method.maghribAngle != null) {
      maghrib = _calculateTimeForAngle(date, latitude, longitude, method.maghribAngle!, ishan: true);
    } else {
      maghrib = sunTimetable.sunsetEnd;
    }

    final dhuhr = sunTimetable.solarNoon;
    final asr = _calculateAsrTime(date, latitude, longitude, madhab: madhabValue);

    return SalahTimes(
      fajr: DateTimeRange(start: fajr, end: sunTimetable.sunriseStart),
      ...
      witr: DateTimeRange(start: isha.add(const Duration(hours: 1)), end: fajr.add(const Duration(days: 1))),
      duha: DateTimeRange(start: sunTimetable.sunriseEnd.add(const Duration(minutes: 20)), end: dhuhr),
      awwabin: DateTimeRange(start: maghrib.add(const Duration(minutes: 20)), end: isha),
      tahajjud: DateTimeRange(start: sunTimetable.nadir, end: fajr.add(const Duration(days: 1))),
    );
  }

  static DateTime _calculateTimeForAngle(DateTime date, double latitude, double longitude, double angle, {bool ishan = false}) {
    ...
    final ha = _radToDeg(acos((sin(_degToRad(-angle)) - sin(_degToRad(latitude)) * sin(_degToRad(declination))) / (cos(_degToRad(latitude)) * cos(_degToRad(declination)))));
  ...
  static DateTime _calculateAsrTime(DateTime date, double latitude, double longitude, {int madhab = 1}) {
    final jday = _getJulianDay(date);
    final jcentury = (jday - 2451545.0) / 36525.0;
    ...
    final asrAlt = _radToDeg(atan(1 / (madhab + tan(_degToRad(latitude - declination).abs()))));
    final ha = _radToDeg(acos((sin(_degToRad(asrAlt)) - sin(_degToRad(latitude)) * sin(_degToRad( declination))) / (cos(_degToRad(latitude)) * cos(_degToRad(declination)))));
  ...
  static DateTime _getDateTime(DateTime date, double decimalHours) {
    ...
    return DateTime.utc(date.year, date.month, date.day, hour.floor(), minute.floor(), second.floor());
  }
}
